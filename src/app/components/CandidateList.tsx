...
function KYCRequiredModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  return (
    ...
    >
      ...
      />
      <div
        className={`relative bg-white rounded-lg shadow-lg p-8 max-w-sm w-full text-center z-10 transform transition-all duration-300 ${
      ...
      >
        ...
        <p className="mb-6 text-gray-700">
          You must complete KYC to participate in voting. Please verify your identity to continue.
        </p>
        <button
  ...
  const handleVote = async (option: Candidate) => {
    ...
    await toast.promise(
      new Promise<void>(async (resolve, reject) => {
        try {
          const tx = await vote(program!, publicKey!, option.pollId, option.cid)
        ...
        <div className="space-y-2">
          {candidates.map((option) => (
            <div
              ...
              className="flex justify-between items-center border-b border-gray-700 last:border-none pb-4 last:pb-0"
            >
